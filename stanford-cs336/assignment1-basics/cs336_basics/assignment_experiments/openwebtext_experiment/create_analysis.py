#!/usr/bin/env python3
"""
Create OpenWebText vs TinyStories Analysis
...
def load_metrics_csv(csv_path):
    ...
    data = {'step': [], 'train_loss': [], 'val_loss': [], 'learning_rate': []}

    with open(csv_path, 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            data['step'].append(int(row['step']))
            data['train_loss'].append(float(row['train_loss']) if row['train_loss'] else None)
            data['val_loss'].append(float(row['val_loss']) if row['val_loss'] else None)
            data['learning_rate'].append(float(row['learning_rate']) if row['learning_rate'] else None)
...
def load_experiment_data(exp_dir):
    ...
    # Load metrics
    metrics = load_metrics_csv(exp_dir / "metrics.csv")
...
def create_comparison_plots(owt_metrics, ts_metrics, owt_summary, ts_summary, output_path):
    ...
    ax = axes[0, 1]
    owt_val_steps = [s for s, v in zip(owt_metrics['step'], owt_metrics['val_loss']) if v is not None]
    owt_val_losses = [v for v in owt_metrics['val_loss'] if v is not None]
    ts_val_steps = [s for s, v in zip(ts_metrics['step'], ts_metrics['val_loss']) if v is not None]
    ts_val_losses = [v for v in ts_metrics['val_loss'] if v is not None]

    ax.plot(owt_val_steps, owt_val_losses,
            label='OpenWebText', color='#e74c3c', marker='o', linewidth=2, markersize=6)
    ax.plot(ts_val_steps, ts_val_losses,
